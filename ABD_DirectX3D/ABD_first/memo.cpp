...
// 
// 프로젝트의 포함한다 : 빌드할때 읽는다.
// 프로젝트의 포함하지 않는다 : 빌드할때 읽지않는다.
// 
// 
// ************과제 ********************
// 1. MousePicking을 Terrain에 적용시키기!
// 2. 마우스를 클릭하면 그 위치로 로보트가 올라가도록 설정하기
// 
// 

#pragma endregion

#pragma region 26Day - CS: Compute Shader 
// 
// CS : compute Shader
// - 계산하는 쉐이더로 rendering pipeline에 포함되는 과정은 아니다.
// -- GPU는 병렬 계산에 특화 되어있다. ex)  mousepicking에 경우만 해도 폴리곤 하나하나에 점을 찍어보면서 충돌인지 아닌지 체크한다.
// -- 이러한 많은 연산을  compute shader에게 넘겨서 계산하게 되면 속도가 빨라진다.
// 
//  frame과 주사율
// - Frame : 데이터를 갱신하는 빈도! == 업데이트 횟수 
// -- 업데이트에 화면정보가 들어가 있을 뿐 화면을 갱신시키는 횟수는 아님
// 
// - 모니터 주사율 (sacanning rate) : 화면을 갱신시키는 비율
// -- 화면을 갱신시키는 비율이므로 업데이트가 되어 정보가 있더라도 화면은 아직 그리지 않았을 수 도 있음
// -- 
// 
// 수직동기화 : frame과 주사율을 맞춰줘서 화면 갱신되는 만큼만 업데이트를 진행하는 것 
// -- 데이터를 갱신하더라도 어차피 화면갱신이 안되기에 시스템에서 막는 것임
// -- 수직 동기화를 끄기만 해도 모니터 주사율과 상관없이 frame을 뽑을 수 있음
// -- 수직동기화를 풀게 되면 싱크가 안맞는 문제가 발생함
// -- ex) 15FPS와 10Hz가 만나면 위에는 갱신되도 아래는 갱신안되는 티어링 현상이 발생가능
// ---  버퍼에다가 그림을 그리는것은 연산은 GPU, 그림은 CPU가 그린다고 보면 됨
// 
// 
// 
// 
